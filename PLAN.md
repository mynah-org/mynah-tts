...
- [~] E15-5 resident Pocket SEANet/decoder streaming: single-context causal conv/residual/transpose-conv state, persistent device scratch, PCM handoff, graph capture and a CPU-reference model-free CUDA self-test are implemented; async gang submit/one-drain scheduling is now present, while true cross-request kernel batching remains open
- [~] E15-6 CUDA server integration: one process/GPU build boundary, prefork refusal and existing queue/cancel/stream contract are present; `/health`, `/metrics` and SIGUSR1 stats expose graph/H2D/D2H/decoder/fallback plus request timing counters, while per-stage GPU timing and real-GPU validation remain
...
- [~] E15-10 resident decoder gang submission and true batch kernels: the first async gang path removes per-request sync and records batch counters; stateful `(batch, frames)` SEANet kernels with packed ConvTranspose/GEMM variants and per-request causal state remain
- [ ] E15-11 CUDA precision/fusion ladder: resident BF16/FP16 weights, cuBLASLt/grouped GEMM, fused bias/activation/QKV/attention/FFN paths, and explicit parity-gated flags; no fast-math claim from the existing FP32-input path
- [ ] E15-12 CUDA graph runner: warmup, padded multi-axis buckets, output trimming, capture/replay/fallback reasons, cache memory and per-bucket hit/miss counters
- [~] E15-13 server observability: read-only `/metrics` now exposes queue/active/completion gauges/counters, active capacity, transfer calls/bytes, sync/graph/decoder-batch/matmul counters, VRAM/backend flags and TTFA/E2E/RTF/audio timing sums; TTFB, per-stage timing and bucket histograms remain
- [~] E15-14 runtime CUDA CI: hosted `nvcc` compile gates now fail on unexpected self-test errors while accepting an explicit no-device result; optional GPU/self-hosted parity, compute-sanitizer and B1/B2/B4/B8 graph/precision matrix remain
- [~] E15-19 real NVIDIA bring-up: RTX PRO 6000 `sm_120` model-free self-test passes and `sm_89` (L4/L40S) compiles; model-backed CUDA parity, transfer/CPU-utilization audit and production qualification remain
