...
- [ ] E11-10 **the item found a better lever than it went looking for** → new candidate for E3-5c/E9-12: `.work/backbone-bandwidth.md` called fewer weight bytes "a quality problem wearing a performance problem's clothes". Measured: backbone traffic per AR step **bf16 151.0 MB → int8 75.7 MB → ternary 19.1 MB**, and int8 passed **0/7** termination failures at both temperatures with the level correct. int8 halves the traffic on the region that owns 48.7% of the wall, **with kernels that already exist**. A screen may not promote — this needs listening plus a 30-min soak before `POCKET_QG_DEFAULT_SPEC`'s `backbone:bf16` pin changes
