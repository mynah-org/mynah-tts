...
- [~] E15-5 resident Pocket SEANet/decoder streaming: single-context causal conv/residual/transpose-conv state, persistent device scratch, device Mimi→SEANet handoff, graph capture and a CPU-reference model-free CUDA self-test are implemented; raw-F32 quantizer/upsample is resident when eligible, while true cross-request SEANet kernel batching remains open
...
- [~] E15-10 resident decoder gang submission and true batch kernels: the first async gang path removes per-request sync and records batch counters; the Mimi decoder-transformer now has a resident cross-request frame-tile path, while stateful `(batch, frames)` SEANet kernels with packed ConvTranspose/GEMM variants and per-request causal state remain
...
- [~] E15-21 CUDA async lifetime/fallback audit: independent frame/layer/request metadata tables, pinned singleton staging, race-free causal upsample-tail state (including tail/stride edge cases), teardown draining, causal upsample-tail import and explicit 1.5.0 metrics ABI bump are implemented; fresh nvcc, compute-sanitizer and injected mid-stream failure runs remain
- [~] E15-22 CUDA capability truthfulness: startup reports raw-F32 stage eligibility and quantized groups stay on the CPU oracle; CUDA qmat formats, EOS/RNG/control-seam residency and true cross-request SEANet arithmetic remain open
