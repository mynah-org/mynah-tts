...
- [~] E15-26 Pocket 24L CPU serving on Axion (16x2 mb8, int8 backbone): **C96 qualified GOOD** (30 min, 43113 req, 0/0 stalls, TTFA p95 282 ms, RTF p95 0.763, 126 audio-s/s) from ~C48 via text segmentation (C64), the wide I8MM kernel K2 (C80) and pooled attention K4 (C96), branch `pocket-24l-cpu-segments`; C104-C112 are a rare-stall edge; K3/K5 rejected. Next: codec conv campaign (stop below ~3% serving gain), productise segmentation → [`.work/pocket-tts-24l-cpu-serving-axion.md`](.work/pocket-tts-24l-cpu-serving-axion.md)
