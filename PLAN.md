...
- [~] E15-4 resident Pocket prefill/AR batch: per-request KV, batched matmul→matmat, device transformer step, cross-request attention, pinned batch staging, bounded host K/V shadow commit and per-scratch `(batch-width)` CUDA-Graph capture/replay buckets are implemented; flow/latent control remains
